...
- 文全体は45字以上。Issue の3文は句点を除いて49〜70字で、文長の下限を30字まで下げても99文書で10件しか出なかった。45字で3文をすべて残す。1文に複数あれば節の数が最も多い並びを1件返す。

- ラベル付きサンプル（`eval/labeled/buried_question_list/`、Issue の文と同じ構造で内容を差し替えた）: fire 10/10（Wilson 95% CI 0.722–1.000）、silent 0/10（CI 0.000–0.278）。事前登録条件（fire/silent各5件以上、fprのWilson上限0.30以下）を満たす。
- 実文書99件（コミット済み18件と外部取得81件、28,439文、各文書のジャンル）: 8件（人間5文書5件、AI 2文書3件）。人間側は寺田寅彦・中島敦の随筆と技術記事3件で、抜粋はいずれも「〜か、〜かを／は」の型を正しく指していた。直すかは著者の判断に残る。
- v0.3.9との比較: 同じ99文書で、ほかの読解負荷カテゴリの件数は変わらない。

残る限界。「〜のか、〜のか」を読点なしで並べる文、節の中に読点を含む文、疑問節を三つ以上並べて途中に別の読点を挟む文は取り逃す。係り先の遠さそのものは測らず、字数と構造で候補にする。見落とし率と、指摘に従って直した文の改善率は未評価。
