...
最初の計測では、99文書の逐次実行でSuikoが14.9秒、Natural Japaneseが14.7秒と並んだ。Natural Japaneseが使うSudachiPyはsudachi.rs（Rust）のPythonバインディングで、処理の大半を占める形態素解析は両者とも同じRustのコードで動く。起動の固定費は、1文の入力でSuikoが0.01秒、Natural Japaneseが0.05秒で、どちらも小さい。

差が出たのは大きな文書である。16万字の白書では、Suikoが1.6秒、Natural Japaneseが1.0秒で、Suikoのほうが遅かった。Suikoの通常の診断だけなら0.31秒で、読解負荷の処理に約1.3秒かかっていた。原因は、文の長さを数える`reading_length`が、文ごとに正規表現をコンパイルし直していたことだった。一度だけコンパイルするよう直すと、この文書は0.40秒になった。

修正後に、ディスクキャッシュが温まった状態で2回ずつ測り直した（Apple Silicon、逐次実行）。

| 実行 | 99文書の所要時間 |
|---|---|
| Natural Japanese（`--reading-load`） | 9.3〜11.5秒 |
| Suiko v0.3.9（`--reading-load`） | 9.7秒（1回） |
| Suiko v0.3.10（読解負荷は既定で有効） | 3.7〜4.4秒 |

計測は1台の手元環境で、並列実行やメモリ使用量は比べていない。
